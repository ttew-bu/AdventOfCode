...
class Part01 extends SolutionDay03{
    ...
    def getSolution(path : String) : Int ={
        ...
        var validStringPortions = fileContentGenerator.map(line => mulRegexPattern.findAllIn(line).toList).toList
...
class Part02 extends SolutionDay03{
    def getSolution(path: String) : Int = {
        ...
        var validStringPortions = validDoSubstrings.map(line => mulRegexPattern.findAllIn(line).toList).toList
        cleanAndMultiply(validStringPortions)
    }
}
